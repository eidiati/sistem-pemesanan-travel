@if ($paginator)
    <nav role="navigation" aria-label="{{ __('Pagination Navigation') }}" class="flex items-center justify-between">
        {{-- Pagination Mobile (Previous Button) & () --}}
        ...
        <div class="hidden sm:flex-1 sm:flex justify-between sm:items-center space-x-6">
            <div class="">
                <p class="text-sm text-gray-700 leading-5 dark:text-gray-400">
                    {!! __('Showing') !!}
                    @if ($paginator->firstItem())
                        <span class="font-medium"><b>{{ $paginator->firstItem() }}</b></span>
                        {!! __('to') !!}
                        <span class="font-medium"><b>{{ $paginator->lastItem() }}</b></span>
                    @else
                        {{ $paginator->count() }}
                    @endif
                    {!! __('of') !!}
                    <span class="font-medium"><b>{{ $paginator->total() }}</b></span>
                    {!! __('results') !!}
                </p>
            </div>
